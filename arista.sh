...
get_ips() {
    ...
    ips=$(curl -sL --max-time 5 "https://raw.githubusercontent.com/Ptechgithub/warp/main/endip/install.sh" 2>/dev/null | bash 2>/dev/null | grep -o '[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*' | head -$count | sort -u)
    
    if [ -z "$ips" ]; then
        ips=$(curl -sL --max-time 5 "https://raw.githubusercontent.com/v2rayA/v2rayA/main/ip.txt" 2>/dev/null | grep -o '[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*' | head -$count | sort -u)
    fi
    
    if [ -z "$ips" ]; then
        ips=$(curl -sL --max-time 5 "https://raw.githubusercontent.com/alanhg/CloudflareSpeedTest/master/ip.txt" 2>/dev/null | grep -o '[0-9]*\.[0-9]*\.[0-9]*\.[0-9]*' | head -$count | sort -u)
    fi
...
measure_latency() {
    local ip=$1
    local latency=$(ping -c 1 -W 2 $ip 2>/dev/null | grep 'time=' | cut -d'=' -f2 | cut -d' ' -f1)
    if [ -n "$latency" ]; then
        echo "$ip|$latency"
    fi
}
...
scan() {
    ...
    if [ -n "$results" ]; then
        ...
        echo -e "$results" | sort -t'|' -k2 -n | head -10 | while IFS='|' read -r ip latency; do
            idx=$((idx+1))
            if [ $(echo "$latency < 50" | bc 2>/dev/null) -eq 1 ]; then
                color=$GREEN
            elif [ $(echo "$latency < 100" | bc 2>/dev/null) -eq 1 ]; then
                color=$YELLOW
            else
                color=$RED
            fi
